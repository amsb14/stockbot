...
# ─── Test Universe ─────────────────────────────────────────────────────────────
COMPANIES = ["1010.SR", "2222.SR"]  # adjust or import from config as needed
...
def insert_cashflows(rows):
    # Remove duplicate primary keys (ticker, Statement_Type, fiscal_date)
    unique = {}
    for row in rows:
    ...
    try:
        ...
        # If autocommit is disabled, uncomment the next line:
        # conn.commit()
    finally:
        put_db_conn(conn)
...
def refresh_cashflow_test():
    """
    Fetch & upsert cashflow on demand.
